...
Replicate 2: Four pinyon jays (1 female) completed all rounds of the food experiment and 10  jays (1 female) completed all rounds of the social experiment. A further 12  jays (5 female) acted as stooge conspecifics in the social experiment. 
...
Each subject experienced five trials (replicate 1) or ten trials (replicate 2) for each of the numerical pairs between 1 and 6 (e.g., 6 vs 5, 6 vs 4, 6 vs 3, etc.) in a pseudo-randomized order. The side of the larger option was pseudo-randomized with no left or right runs longer than three consecutive trials. Subjects experienced one trial per day are were tested in a randomized order each day.    
...
To investigate our other two hypotheses, we used model selection with mixed effect models. Hypothesis two posits that pinyon jays prefer higher differences and lower ratios, which would predict both difference and ratio are included as main effects in the best-fitting model. For both replicates, the best-fitting random effect structure was no random effect structure. When comparing five fixed effect model structures, the model with only the main effect of ratio (Replicate 1: `r printbf(food1_results$fixed_table$BF[2])`, Replicate 2: `r printbf(food2_results$fixed_table$BF[2])`) best fit both data sets (see Tables ?? and ?? for full results). Thus, in both replicates of the food study, subjects had stronger preferences for larger options at smaller ratios (Figure \ref{fig:foodgraphs}A&C). Our third hypothesis was not supported, since difference was not included in the best fitting model (Figure \ref{fig:foodgraphs}B&D).
...
## Social Experiment
Hypothesis 1 predicted that subjects would choose the larger number of flock mates over the smaller. One sample t-tests provided moderate evidence that our hypothesis was supported for replicate 1 (`r apa_print(social1_results$ttest)$full_result`, `r apa_print(social1_results$ttestbf)$statistic`). However, evidence supported no difference from chance replicate 2 (`r apa_print(social2_results$ttest)$full_result`, `r apa_print(social2_results$ttestbf)$statistic`). <!--This is the only incongruency between the replicates. The slight preference for the larger side is likely not a true preference for larger _amounts_ but rather that a preferred partner is more likely to appear in a larger group. To account for this, we completed 10 replications of each pair instead of 5 in replication 2.-->

For hypotheses 2 and 3, we again used mixed effect model selection. No random effect structure was found for both replicates.  For fixed effects, the intercept only model (Replicate 1: `r printbf(social1_results$fixed_table$BF[2])`, Replicate 2: `r printbf(social2_results$fixed_table$BF[2])`) best fit the data, suggesting that neither ratio nor difference influenced choice. Because ratio and difference failed to predict choices, neither of our hypothesis were supported by the data (Figure \ref{fig:socialgraphs}).
