...
abstract: >
 Animals must often discriminate different quantities of objects in their environment, from numbers of food items to conspecifics. Yet we know little about how numerical cognitive abilities compare across different object types. Based on past research, we would expect individuals to use both the numerical difference (large $-$ small) and numerical ratio (small/large) between two numbers of objects to discriminate between them. This study investigates whether numerical difference and ratio predict preferences for two types of stimuli, food items and conspecifics in pinyon jays (_Gymnorhinus cyanocephalus_). Subjects (N=12 for food experiment, N=20 for social experiment) chose between two food items or two group sizes for pairs of numbers between 1 and 6. In the food experiment, pinyon jays showed an overall preference for the larger option over the smaller option. Pinyon jays also chose larger numbers of items more when numerical differences were large and numerical ratios were small. However, numerical difference and ratio did not influence food choice independently. The social experiment showed different results. When choosing between groups of conspecifics, pinyon jays did not prefer the larger over smaller group sizes. Birds also did not show numerical difference or ratio effects. Therefore, pinyon jays may use different cognitive processes when deciding between numbers of food items and flock mates. While number was important for selecting food items, other factors such as flock mate identity may be more important for selecting social groups to join. Thus, the type of objects offered can drive the processes that animals use to choose among quantities.
...
We conducted experiments to investigate quantification of both food and conspecifics. Each experiment was replicated with two sets of birds, where most birds experienced both the food and social experiment. Additional methods, data analysis, and visuals are available in the supplementary material. 
...
The apparatus was a Y maze where the subject entered a large chamber at the base of the maze and could choose one of two arms. Doors separated the entrance chamber from the arms, where large bird cages housed the stooge birds (Figure S2).
...
All data were analyzed using the same pre-registered analyses (https://aspredicted.org/RVH_MNB) in R version `r paste0(R.version$major, ".", R.version$minor)` [@R-base]. To test the first hypothesis, whether pinyon jays prefer larger over smaller numbers of food items and conspecifics, we conducted frequentist and Bayesian one sample t-tests. 

To test our second and third hypotheses, whether numerical difference and ratio predict preferences between smaller and larger options independently, we used generalized linear mixed-effects modeling. We used trial-level choices for either the larger or smaller option available in the number pair as the response variable. To investigate our hypotheses, we used model selection to compare which combination of random (subject, pair, or both) and fixed (ratio, difference, or a combination of both) effects best describe each data set (food and social)(Table S3). We draw inferences based on Bayesian statistics (BF$_{10}$ values) where a BF$_{10}$ > 3 is sufficient evidence for the alternative hypothesis, BF$_{10}$ < 1/3 is sufficient evidence for the null hypothesis, and 1/3 < BF$_{10}$ < 3 indicate neither hypothesis has evidence supporting it, suggesting the sample size is too small to draw conclusions [@Wagenmakers.etal.2018].
...
[**Need to introduce what this analysis is**] For both replicates, the best-fitting random effect structure was no random effect structure. For the fixed effect model comparison, the model with only the main effect of ratio best fit both data sets (Replicate 1: `r printbf(food1_results$fixed_bf_df$BF[2])`, Replicate 2: `r printbf(food2_results$fixed_bf_df$BF[2])`). Thus, subjects in the food experiment used the ratio between the two numbers of mealworms to choose between options, with stronger preferences for larger options at smaller ratios (Figure \ref{fig:foodgraphs}). The model with the main effect of difference had extreme evidence supporting stronger preferences for larger options at larger differences for replicate 1 but not replicate 2 (Replicate 1: `r printbf(food1_results$fixed_bf_df$BF[3])`, Replicate 2: `r printbf(food2_results$fixed_bf_df$BF[3])`). Consequently, this only partially supports our second hypothesis (Figure \ref{fig:foodgraphs}).  Additionally, our third hypothesis was not supported, as the interaction model with difference and ratio did not have a BF$_{10}$ > 3 in either replication (see Tables S4 and S5 for full results). 
...
## Social Experiment
Hypothesis 1 predicted that subjects would choose the larger number of flock mates over the smaller. One sample t-tests provided strong evidence that our hypothesis was supported in replicate 1 (`r apa_print(social1_results$ttest)$full_result`, `r apa_print(social1_results$ttestbf)$statistic`). However, evidence supported no difference from chance in replicate 2 (`r apa_print(social2_results$ttest)$full_result`, `r apa_print(social2_results$ttestbf)$statistic`). 
...
One possible explanation for the lack of a ratio or difference effect for the social preference task is that individual identity of birds overrides the importance of number. An exploratory follow-up analysis of our data showed wide variation in preferences for groups that contained individual stooge birds (Table S6; Table S7). Pinyon jays have complex, long-term bonds with other flock members and mates [@Marzluff.Balda.1992], which may make identity of group mates more important than sheer numbers. Moreover, the birds in our studies did not experience signals of predation danger during the experiment. Without pressure to dilute risk in larger groups, the birds may have ignored group size, allowing them to use other information such as social partner identity to determine choice.
