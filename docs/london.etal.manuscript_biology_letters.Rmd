...
We conducted both a food experiment and a social experiment to investigate quantification of food and conspecifics. Each experiment was replicated with the opposite set of birds so most birds saw both the food and social condition.
...
All data were analyzed using the same pre-registered analyses (**give preregistration links**) in R version `r paste0(R.version$major, ".", R.version$minor)` [@R-base]. To test the first hypothesis, whether pinyon jays prefer larger over smaller numbers of food items and conspecifics, we conducted frequentist and Bayesian one sample t-tests (using the `ttestBF()` function from the _BayesFactor_ package with default, noninformative priors [@R-BayesFactor]). 
...
## Social Experiment
Hypothesis 1 predicted that subjects would choose the larger number of flock mates over the smaller. One sample t-tests provided moderate evidence that our hypothesis was supported during the first replicate (`r apa_print(social1_results$ttest)$full_result`, `r apa_print(social1_results$ttestbf)$statistic`). However, evidence supported no difference from chance in the second replicate (`r apa_print(social2_results$ttest)$full_result`, `r apa_print(social2_results$ttestbf)$statistic`). This is the only incongruency between the replicates. The slight preference for the larger side is likely not a true preference for larger _amounts_ but rather a preferred partner is more likely to appear in the larger group. To account for this, we completed 10 replications of each pair instead of 5 in replication 2.

For hypotheses 2 and 3, we again used mixed effect model selection. The data again supported no random effect structure for both replicates.  For fixed effects, the intercept only model (Replicate 1: `r printbf(social1_results$fixed_table$BF[2])`, Replicate 2: `r printbf(social2_results$fixed_table$BF[2])`) again best fit the data, suggesting that neither ratio nor difference influenced choice. Because ratio and difference failed to predict choices, neither of our hypothesis were supported by the data (Figure \ref{fig:socialgraphs}).
