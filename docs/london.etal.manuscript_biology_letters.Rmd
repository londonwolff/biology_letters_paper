...
## Social Experiment
Hypothesis 1 predicted that subjects would choose the larger number of flock mates over the smaller. <!--During the first replicate, our subjects choose the larger option r mean(birdsummary_social$perc)% of the time +/- r sd(birdsummary_social$perc) and r mean(birdsummary_social_rep$perc)% of the time +/- r sd(birdsummary_social_rep$perc) during the second replicate.--> One sample t-tests provided moderate evidence that our hypothesis was supported during the first replicate (`r apa_print(social1_results$ttest)$full_result`, `r apa_print(social1_results$ttestbf)$statistic`). However, evidence supported no difference from chance in the second replicate (`r apa_print(social1_results$ttest)$full_result`, `r apa_print(social1_results$ttestbf)$statistic`). This is the only incongruency between the replicates and can most likely be accounted for that the slight preference for the larger side is not a true preference for larger _amounts_ but that a preferred partner is more likely to appear in the larger group. This was accounted for in replicate 2 with completing 10 replications of each pair instead of 5.

For hypotheses 2 and 3, we tested all possible random effects (subject, pair, and both) against the intercept only model to determine which of these random effects should be included in the full model. The data again supported the same best fitting model structure for both replicates of data: no random effect structure. <!--The intercept only  model with a Bayes factor of (Replicate 1: `r social_random_comparison_table$BF[1]`, Replicate 2: `r social_random_comparison_rep_table_rep$BF[1]` ) is the chosen random effect structure by rule of parsimony as it is the least complex model, therefore no random effect structure was added to our fixed effect models for the social study.--> For fixed effects, the intercept only model (Replicate 1: `r printbf(social1_results$fixed_table$BF[2])`, Replicate 2:`r printbf(social2_results$fixed_table$BF[2])`) again best fit the data, suggesting that neither ratio nor difference influenced choice. Because ratio and difference failed to predict choices, neither of our hypothesis were supported by the data (Figure \ref{fig:socialgraphs}).
