...
Upon completion of the preregistered analysis we conducted an exploratory examination of subject bird preference for individual stooge birds. We first calculated if each stooge bird was on the side chosen by each subject bird per trial. A binary operator (0 for not chosen, 1 for chosen) was used to note if a specific stooge bird was chosen for each trial. We then created a binary operator for if each stooge bird was offered on that particular trial. For instance, if Zappa was not chosen during a particular trial he was only *not* chosen if he was offered at all in either choice. Using these results we calculated how often each stooge bird was chosen by each sex of subject birds (@fig-preferencetable). Visualizing how often male and female subject birds choose each stooge bird. The female, Sapphire, was chosen 78.6% of the time she was on offer for male subject birds as appose to the 64.3% of the time for female subject birds. 
