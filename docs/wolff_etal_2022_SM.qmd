...
Upon completion of the preregistered analysis, we conducted an exploratory examination of subject bird preferences for individual stooge birds. We first calculated the percentage of trials that each subject selected the side for each stooge separately for both replicates. @fig-individualpref shows these percentages with subject and stooge sex indicated. For example, the subject bird Black Elk choose Sapphire `r printnum(pluck(filter(heatmap_df_long, stooge == "Sapphire", subject == "Black_Elk"), 4), digits = 1)`% of the time that she was offered as an option during the replicate 1. Alternatively, Juniper choose the stooge bird Hermia `r printnum(pluck(filter(heatmap_df_long, stooge == "Hermia", subject == "Juniper"), 4), digits = 1)`% of the time she was an option during replicate 1. Due to bird colony facility maintenance, we used more stooge birds in replicate 1 than replicate 2. Therefore, birds met fewer times in replicate 1 but met more birds overall (@tbl-stoogeinfotable).

We calculated how often each stooge bird was chosen by aggregating over each sex of subject birds (@tbl-preferencetable) to visualize how often male and female subject birds choose each stooge bird. For instance, the most preferred stooge was the female, Sapphire, who was chosen `r printnum(pluck(filter(individual_preference_table, stooge == "Sapphire" & rep == 1), 10), digits = 1)`% of the time she was on offer for male subject birds and `r printnum(pluck(filter(individual_preference_table, stooge == "Sapphire" & rep == 1), 9), digits = 1)`% of the time for female subject birds. 
...
#| fig-cap: "Preference for individual stooges for replicates 1 and 2."
#| out-width: "100%"

include_graphics(here("figures/individual_preference.png"))
...
```{r tbl-stoogeinfotable}

kable(stooge_bird_info,
      booktabs = TRUE, digits = 1, linesep = "",
        align = "llccc",
        caption = "Subject bird information",
        col.names = c("Subject", "Sex", "Age (years)", "Social 1 trials", "Social 2 trials"))
...
```{r tbl-preferencetable}
individual_preference_table |> 
  select(stooge, total_trials, female_percent, male_percent, overall_percent) |> 
