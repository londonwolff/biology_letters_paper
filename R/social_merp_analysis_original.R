...
messy_bird_placement_data <- c(12.8, 17.11, 20.6, 23.11, 31.3, 41.3, 51.8,69.6,84.5,90.6,99.4,100.0)

df_social <- data_phase_1 %>%
  select(id, subject_bird, small_num, large_num, large_side, choice, initials, date, time_in, time_out) %>%
  filter(!(id %in% messy_bird_placement_data)) %>%
  drop_na()
...
df_social <- df_social %>%
  ...
  filter(subject_bird != "Baloo", small_num != 0)

#create bird sex column

female_birds_social <- c("Robin", "Juniper", "Hippo", "Flute")
